...
export default function DeliveryOrders() {
  ...
  useEffect(()=>{
    if(isOwner(authedUser)){
        GetOrdersByStatus(
          {ownerId: getOwnerId(authedUser), status: 'new', mode: 'DELIVERY'},
           (data)=>{
            const orders = Object.values(data)
            setPendingOrders(orders.filter((item)=>moment(item.orderAt).isSame(new Date(), sort)))
          })
        GetOrdersByStatus(
          {ownerId: getOwnerId(authedUser), status: 'accepted', mode: 'DELIVERY'},
          (data)=>{
            const orders = Object.values(data)
            setAcceptedOrders(orders.filter((item)=>moment(item.orderAt).isSame(new Date(), sort)))
          })
        GetOrdersByStatus(
          {ownerId: getOwnerId(authedUser), status: 'ready', mode: 'DELIVERY'},
          (data)=>{
            const orders = Object.values(data)
            setReadyOrders(orders.filter((item)=>moment(item.orderAt).isSame(new Date(), sort)))
          })
        GetOrdersByStatus(
          {ownerId: getOwnerId(authedUser), status: 'shipping', mode: 'DELIVERY'},
          (data)=>{
            const orders = Object.values(data)
            setShippingOrders(orders.filter((item)=>moment(item.orderAt).isSame(new Date(), sort)))
          })
        GetOrdersByStatus(
          {ownerId: getOwnerId(authedUser), status: 'completed', mode: 'DELIVERY'},
          (data)=>{
            const orders = Object.values(data)
        ...
        GetOrdersByStatusAndShop(
          { status: 'new', shopId: authedUser.shop, mode: 'DELIVERY'},
          (data)=>{
            const orders = Object.values(data)
            setPendingOrders(orders.filter((item)=>moment(item.orderAt).isSame(new Date(), sort)))
          })
        GetOrdersByStatusAndShop(
          { status: 'accepted', shopId: authedUser.shop, mode: 'DELIVERY'},
          (data)=>{
            const orders = Object.values(data)
            setAcceptedOrders(orders.filter((item)=>moment(item.orderAt).isSame(new Date(), sort)))
          })
        GetOrdersByStatusAndShop(
          { status: 'ready', shopId: authedUser.shop, mode: 'DELIVERY'},
          (data)=>{
            const orders = Object.values(data)
            setReadyOrders(orders.filter((item)=>moment(item.orderAt).isSame(new Date(), sort)))
          })
        GetOrdersByStatusAndShop(
          { status: 'shipping', shopId: authedUser.shop, mode: 'DELIVERY'},
          (data)=>{
            const orders = Object.values(data)
            setShippingOrders(orders.filter((item)=>moment(item.orderAt).isSame(new Date(), sort)))
          })
        GetOrdersByStatusAndShop(
          { status: 'completed', shopId: authedUser.shop, mode: 'DELIVERY'},
          (data)=>{
            const orders = Object.values(data)
            setDeliveredOrders(orders.filter((item)=>moment(item.orderAt).isSame(new Date(), sort)))
          })
  ...
  const handleShipOrder = useCallback((orderId)=>{
    SetOrderStatus({status: 'shipping', orderId})
  },[])
  const handleSelectOption = (value)=>setSort(value)
  return (
    <Page title="Delivery Orders | Tchopify">
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <HeaderBreadcrumbs
          heading={t('allOrders.deliveryTitle')}
          links={[
            { name: t('links.dashboard'), href: PATH_DASHBOARD.root },
            { name: t('links.deliveryOrders'), href: PATH_DASHBOARD.order.all },
          ]}
        />
            ...
            <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={4}>
                    ...
                    data={readyOrders}
                    action1={{onAction: handleShipOrder, title: t('actions.shipping'), visible: false}} 
                     action2={{ visible: false}} 
                    />
                </Grid>
                <Grid item xs={12} md={4}>
                    <OrderCaroussel 
                    title={t('allOrders.titleShipping')}
                    data={shippingOrders}
